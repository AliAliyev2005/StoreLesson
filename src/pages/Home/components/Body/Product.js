...
function Product({ data }) {
    ...
    return (
        <Box
            ...
            h={"280px"}
            borderRadius={10}
            _hover={{ boxShadow: "0 4px 8px 0 rgba(177,181,199,0.48)", cursor: "pointer" }}
        >
            <Image
                borderRadius="md"
            ...
            />
            <Box p={5} pt={0} h={"100%"} display={"flex"} flexDir={"column"} justifyContent={"space-between"}>
                <Flex w={"100%"} position={"absolute"} top={0} left={0} px={3} align="baseline" justifyContent={"space-between"}>
                    {data.is_vip ? <Badge colorScheme="yellow">VIP</Badge> : <span></span>}
                    <Flex bg={"white"} borderRadius={4} mt={2} px={1} align="center">
